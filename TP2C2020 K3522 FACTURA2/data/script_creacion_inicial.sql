...
CREATE TABLE [GDD2020].[COMPRA_AUTOPARTE](
	...
	[SUCURSAL_CODIGO] BIGINT,
	[COMPRA_NRO] [decimal](18, 0),
	CONSTRAINT FK_MODELO_CODIGO_COMPRA_AUTOPARTE FOREIGN KEY (MODELO_CODIGO) REFERENCES [GDD2020].[MODELO](MODELO_CODIGO),
	CONSTRAINT FK_FABRICANTE_CODIGO_COMPRA_AUTOPARTE FOREIGN KEY (FABRICANTE_CODIGO) REFERENCES [GDD2020].[FABRICANTE](FABRICANTE_CODIGO),
	CONSTRAINT FK_SUCURSAL_CODIGO_COMPRA_AUTOPARTE FOREIGN KEY (SUCURSAL_CODIGO) REFERENCES [GDD2020].[SUCURSAL](SUCURSAL_CODIGO),
...
CREATE TABLE [GDD2020].[AUTOPARTE](
	...
	CONSTRAINT FK_FAC_AUTOPARTE_CODIGO FOREIGN KEY (FAC_AUTOPARTE_CODIGO) REFERENCES [GDD2020].[FACTURA_AUTOPARTE](FAC_AUTOPARTE_CODIGO),
...
CREATE TABLE [GDD2020].[MOTOR](
	...
	[AUTO_PARTE_CODIGO] [decimal](18, 0),
	CONSTRAINT FK_AUTO_PARTE_CODIGO_MOTOR FOREIGN KEY (AUTO_PARTE_CODIGO) REFERENCES [GDD2020].[AUTOPARTE](AUTO_PARTE_CODIGO)
) ON [PRIMARY] --insertado
...
CREATE TABLE [GDD2020].[TRANSMISION](
	...
	[AUTO_PARTE_CODIGO] [decimal](18, 0),
	CONSTRAINT FK_AUTO_PARTE_CODIGO_TRANSMISION FOREIGN KEY (AUTO_PARTE_CODIGO) REFERENCES [GDD2020].[AUTOPARTE](AUTO_PARTE_CODIGO)
) ON [PRIMARY] --insertado
...
ORDER BY m.COMPRA_NRO --AND m.COMPRA_NRO = 125176

--SELECT DISTINCT m.COMPRA_NRO,su.SUCURSAL_DIRECCION,m.COMPRA_FECHA,cli.CLIENTE_DNI,cli.CLIENTE_APELLIDO,cli.CLIENTE_NOMBRE
--FROM gd_esquema.Maestra m
--JOIN GDD2020.SUCURSAL su
--ON m.SUCURSAL_DIRECCION = su.SUCURSAL_DIRECCION 
--JOIN GDD2020.CLIENTE cli
...
---SELECT cli.CLIENTE_DNI,cli.CLIENTE_APELLIDO,cli.CLIENTE_NOMBRE FROM GDD2020.CLIENTE cli WHERE cli.CLIENTE_DNI = 93569323

--ITEM_COMPRA
INSERT INTO GDD2020.ITEM_COMPRA(COMPRA_PRECIO,COMPRA_CANT,COMPRA_NRO)
...
--FACTURACION | Rompe
INSERT INTO GDD2020.FACTURACION(FACTURA_NRO,SUCURSAL_CODIGO,PRECIO_FACTURADO)
SELECT DISTINCT m.FACTURA_NRO, su.SUCURSAL_CODIGO, SUM(m.PRECIO_FACTURADO)
FROM gd_esquema.Maestra m
JOIN GDD2020.SUCURSAL su
ON m.FAC_SUCURSAL_DIRECCION = su.SUCURSAL_DIRECCION
WHERE FACTURA_NRO IS NOT NULL
GROUP BY m.FACTURA_NRO, su.SUCURSAL_CODIGO

--ITEM_FACTURA probablemente no haga falta.
...
SELECT DISTINCT mo.MODELO_CODIGO,fab.FABRICANTE_CODIGO,su.SUCURSAL_CODIGO,com.COMPRA_NRO
FROM gd_esquema.Maestra m
...
INSERT INTO GDD2020.FACTURA_AUTOPARTE(SUCURSAL_CODIGO,CANT_FACTURADA,FACTURA_NRO)
SELECT DISTINCT su.SUCURSAL_CODIGO,m1.CANT_FACTURADA,m1.FACTURA_NRO
FROM gd_esquema.Maestra m1
JOIN (SELECT FACTURA_NRO,FAC_SUCURSAL_DIRECCION FROM gd_esquema.Maestra WHERE FACTURA_NRO IS NOT NULL AND FAC_SUCURSAL_DIRECCION IS NOT NULL) m2 ON(m2.FACTURA_NRO = m1.FACTURA_NRO)
JOIN GDD2020.SUCURSAL su ON(su.SUCURSAL_DIRECCION = m2.FAC_SUCURSAL_DIRECCION)
WHERE m1.FACTURA_NRO IS NOT NULL AND m1.CANT_FACTURADA IS NOT NULL
...
SELECT DISTINCT m.AUTO_PARTE_CODIGO,m.AUTO_PARTE_DESCRIPCION,ca.CODIGO_COMPRA_AUTOPARTE,fap.FAC_AUTOPARTE_CODIGO,mo.MODELO_CODIGO,fab.FABRICANTE_CODIGO
FROM gd_esquema.Maestra m
JOIN GDD2020.COMPRA_AUTOPARTE ca
ON m.COMPRA_NRO = ca.COMPRA_NRO
JOIN GDD2020.FACTURA_AUTOPARTE fap
ON m.FACTURA_NRO = fap.FACTURA_NRO
JOIN GDD2020.MODELO mo
ON m.MODELO_CODIGO = mo.MODELO_CODIGO
JOIN GDD2020.FABRICANTE fab
ON m.FABRICANTE_NOMBRE = fab.FABRICANTE_NOMBRE
