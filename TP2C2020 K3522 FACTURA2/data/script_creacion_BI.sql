...
AS
SELECT SUM(PRECIO_FACTURADO) - SUM(COMPRA_PRECIO) ganancia, f.SUCURSAL_CODIGO, MONTH(FACTURA_FECHA) mesFactura, YEAR(FACTURA_FECHA) anioFactura
FROM GDD2020.FACTURACION f
JOIN GDD2020.FACTURA_AUTO fa ON (fa.FACTURA_NRO = f.FACTURA_NRO)
JOIN GDD2020.COMPRA c ON (c.SUCURSAL_CODIGO = f.SUCURSAL_CODIGO)
JOIN GDD2020.COMPRA_AUTOMOVIL ca ON (ca.COMPRA_NRO = c.COMPRA_NRO)
JOIN GDD2020.ITEM_COMPRA ic ON (ic.COMPRA_NRO = c.COMPRA_NRO)
GROUP BY f.SUCURSAL_CODIGO, MONTH(FACTURA_FECHA), YEAR(FACTURA_FECHA)
...
AS
SELECT SUM(PRECIO_FACTURADO) - SUM(COMPRA_PRECIO * COMPRA_CANT) ganancia, f.SUCURSAL_CODIGO, MONTH(FACTURA_FECHA) mesFactura, YEAR(FACTURA_FECHA) anioFactura
FROM GDD2020.FACTURACION f
JOIN GDD2020.FACTURA_AUTOPARTE fa ON (fa.FACTURA_NRO = f.FACTURA_NRO)
JOIN GDD2020.COMPRA c ON (c.SUCURSAL_CODIGO = f.SUCURSAL_CODIGO AND MONTH(c.COMPRA_FECHA) = MONTH(f.FACTURA_FECHA) AND YEAR(c.COMPRA_FECHA) = YEAR(f.FACTURA_FECHA))
JOIN GDD2020.COMPRA_AUTOPARTE ca ON (ca.COMPRA_NRO = c.COMPRA_NRO)
JOIN GDD2020.ITEM_COMPRA ic ON (ic.COMPRA_NRO = c.COMPRA_NRO)
GROUP BY f.SUCURSAL_CODIGO, MONTH(FACTURA_FECHA), YEAR(FACTURA_FECHA)
...
AS
SELECT SUM(f.PRECIO_FACTURADO) / COUNT(f.FACTURA_NRO) precioPromedioVendido, SUM(COMPRA_PRECIO) / COUNT(c.COMPRA_NRO) precioPromedioComprado, a.AUTO_PARTE_CODIGO
FROM GDD2020.BI_AUTOPARTE a
JOIN GDD2020.FACTURA_AUTOPARTE_X_AUTOPARTE faxa ON (faxa.AUTO_PARTE_CODIGO = a.AUTO_PARTE_CODIGO)
JOIN GDD2020.FACTURA_AUTOPARTE fa ON (fa.FAC_AUTOPARTE_CODIGO = faxa.FAC_AUTOPARTE_CODIGO)
JOIN GDD2020.FACTURACION f ON (f.FACTURA_NRO = fa.FACTURA_NRO)
JOIN GDD2020.COMPRA_AUTOPARTE_X_AUTOPARTE caxa ON (caxa.AUTO_PARTE_CODIGO = a.AUTO_PARTE_CODIGO)
JOIN GDD2020.COMPRA_AUTOPARTE ca ON (ca.CODIGO_COMPRA_AUTOPARTE = caxa.CODIGO_COMPRA_AUTOPARTE)
JOIN GDD2020.COMPRA c ON (c.COMPRA_NRO = ca.COMPRA_NRO)
JOIN GDD2020.ITEM_COMPRA ic ON (ic.COMPRA_NRO = c.COMPRA_NRO)
...
AS
SELECT SUM(f.PRECIO_FACTURADO) / COUNT(f.FACTURA_NRO) precioPromedioVendido, SUM(COMPRA_PRECIO) / COUNT(c.COMPRA_NRO) precioPromedioComprado, a.AUTO_PATENTE
FROM GDD2020.AUTOMOVIL a
JOIN GDD2020.FACTURA_AUTO fa ON (fa.AUTO_PATENTE = a.AUTO_PATENTE)
JOIN GDD2020.FACTURACION f ON (f.FACTURA_NRO = fa.FACTURA_NRO)
JOIN GDD2020.COMPRA_AUTOMOVIL ca ON (ca.AUTO_PATENTE = a.AUTO_PATENTE)
JOIN GDD2020.COMPRA c ON (c.COMPRA_NRO = ca.COMPRA_NRO)
JOIN GDD2020.ITEM_COMPRA ic ON (ic.COMPRA_NRO = c.COMPRA_NRO)
...
AS
SELECT COUNT(ca.COMPRA_AUTOMOVIL_CODIGO) cantidadCompra,COUNT(fa.FAC_AUTO_CODIGO) cantidadVendida,ca.SUCURSAL_CODIGO,MONTH(f.FACTURA_FECHA) mes, YEAR(f.FACTURA_FECHA) anio
FROM GDD2020.COMPRA_AUTOMOVIL ca
JOIN GDD2020.FACTURA_AUTO fa ON (ca.AUTO_PATENTE = fa.AUTO_PATENTE)
JOIN GDD2020.COMPRA c ON (ca.COMPRA_NRO = c.COMPRA_NRO)
JOIN GDD2020.FACTURACION f ON (fa.FACTURA_NRO = f.FACTURA_NRO)
GROUP BY ca.SUCURSAL_CODIGO, MONTH(f.FACTURA_FECHA), YEAR(f.FACTURA_FECHA)